...
const  posts = [
  {
    ...
    {
    author: "Jane Kabengele",
    title: "How to code à navbar"
  }, 
      {
    author: "Bruce Willis",
    title: "Ceci est un braquage"
  }
]
        ...
        <li key={id} className="rounded-sm shadow-md hover:shadow-xl border hover:border-zinc-300">
          <div className="pt-5 px-5 pb-7">
              <div className="flex items-baseline gap-x-4 text-xs">
                ...
                <time dateTime={new Date().toISOtring}>
                ...
                <Link href={`/categories/author/${post.author}`} className="ml-auto text-base text-gray-700 hover:text-gray-600 whitespace-nowrap truncate">
                  {post.author}
                      ...
                      <Link href={`/article/${post.title}`} className="inline-block mt-6 text-xl font-semibold text-zinc-800 hover:text-zinc-600">
                  {post.title}
                </Link>
          </div>
        </li>
